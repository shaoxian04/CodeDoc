...
export function activate(context: vscode.ExtensionContext) {
  ...
  console.log("CodeDoc extension activation started");
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('codedoc.syncDocs', async (changedFiles?: string[]) => {
            ...
            try {
                ...
                if (!mdFiles || mdFiles.length === 0) {
                    ...
                    const generated = resp.success && resp.data ? (resp.data as string) : '';
                ...
                if (changedFiles && Array.isArray(changedFiles) && changedFiles.length) {
                    ...
                    if (rootReadme && !related.find(u => u.fsPath === rootReadme.fsPath)) related.push(rootReadme);
                ...
                async function lastCommitTimeForFile(filePath: string, cwdPath: string | undefined) {
                    try {
                        if (!cwdPath) return 0;
                ...
                for (const mdUri of filesToCheck) {
                    try {
                        ...
                        const scores: Array<{ uri: vscode.Uri; score: number; snippet: string }> = [];
                        ...
                        if (referenced.length === 0) {
                            ...
                            if (repoLatest) {
                                ...
                                if (!isNaN(sec)) latestCodeCommit = Math.max(latestCodeCommit, sec * 1000);
                            }
                        }

                        // Heuristic: detect if markdown contains tokens that lack descriptions
                        function markdownLacksDescription(markdownText: string, tokens: string[]) {
                        ...
                        const lacksDesc = markdownLacksDescription(existing, tokens);
                            ...
                            if (codeAnnotations.size > 0) {
                                ...
                                for (const ca of codeAnnotations) {
                                    if (!mdAnnotations.has(ca)) { annotationMismatch = true; break; }
                                }
                            }
...
``
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("codedoc.exportDiagramAsImage", async (diagramData: any) => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("codedoc.openDiagramAsImage", async (diagramData: any) => {
...
async function convertMermaidToSvg(mermaidCode: string): Promise<string | null> {
  try {
...
function startGitRemotePoller() {
    ...
    checkOnce();
    const timer = setInterval(() => { if (!stopped) checkOnce(); }, intervalSec * 1000);
