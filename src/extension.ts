...
export function activate(context: vscode.ExtensionContext) {
  ...
  try {
    ...
    console.log(
      "🔥 [DEBUG] Extension activation completed - setting up auto-analysis..."
    );
    console.log("🔥 [DEBUG] Current time:", new Date().toISOString());
    console.log(
      "🔥 [DEBUG] Workspace folders available:",
      vscode.workspace.workspaceFolders?.length || 0
    );

    // Test immediate Java file search
    vscode.workspace.findFiles("**/*.java", "**/node_modules/**", 5).then(
      (files) => {
        console.log(
          "🔥 [DEBUG] Immediate Java file check found:",
          files.length,
          "files"
        );
    ...
    (async () => {
      ...
      try {
        console.log(
          "🚀 [AUTO-ANALYSIS] Starting automatic project analysis on extension startup..."
        );
        ...
        console.log(
          "📂 [AUTO-ANALYSIS] Workspace folders:",
        ...
        if (workspaceFolders && workspaceFolders.length > 0) {
          ...
          workspaceFolders.forEach((folder, index) => {
            console.log(
              `📁 [AUTO-ANALYSIS] Workspace ${index + 1}: ${folder.name} (${
                folder.uri.fsPath
              })`
            );
          });
          ...
          console.log(
            `⏱️ [AUTO-ANALYSIS] File search completed in ${searchDuration}ms`
          );
          console.log(
            `📄 [AUTO-ANALYSIS] Found ${javaFiles.length} Java files`
          );
          ...
          if (javaFiles.length > 0) {
            ...
            javaFiles.slice(0, 5).forEach((file, index) => {
              console.log(
                `📝 [AUTO-ANALYSIS] Java file ${index + 1}: ${file.fsPath}`
              );
            });
            if (javaFiles.length > 5) {
              console.log(
                `📝 [AUTO-ANALYSIS] ... and ${javaFiles.length - 5} more files`
              );
            }

            console.log(
              "🎯 [AUTO-ANALYSIS] Triggering codedoc.visualizeCode command..."
            );
            ...
            console.log(
              `⏱️ [AUTO-ANALYSIS] Analysis command completed in ${analysisDuration}ms`
            );
            console.log(
              `✅ [AUTO-ANALYSIS] Automatic project analysis initiated successfully`
            );
            console.log(
              `🏁 [AUTO-ANALYSIS] Total auto-analysis time: ${totalDuration}ms`
            );

            // Show user notification
            vscode.window.showInformationMessage(
              `CodeDoc: Analyzing ${javaFiles.length} Java files in background...`,
              { modal: false }
            );
          } else {
            console.log(
              "ℹ️ [AUTO-ANALYSIS] No Java files found in workspace, skipping auto-analysis"
            );
            console.log(
              "💡 [AUTO-ANALYSIS] Tip: Make sure your Java files are in the workspace and not in node_modules"
            );
          }
        } else {
          console.log(
            "ℹ️ [AUTO-ANALYSIS] No workspace folders found, skipping auto-analysis"
          );
          console.log(
            "💡 [AUTO-ANALYSIS] Tip: Open a folder containing Java files to enable auto-analysis"
          );
        }
      } catch (error) {
        const totalDuration = Date.now() - startTime;
        console.error(
          "❌ [AUTO-ANALYSIS] Auto-analysis failed after",
          totalDuration + "ms"
        );
        console.error("❌ [AUTO-ANALYSIS] Error details:", error);
        console.error(
          "❌ [AUTO-ANALYSIS] Stack trace:",
          error instanceof Error ? error.stack : "No stack trace"
        );
        console.warn(
          "⚠️ [AUTO-ANALYSIS] Extension will continue normally despite auto-analysis failure"
        );

        // Show error notification to user
        vscode.window.showWarningMessage(
          "CodeDoc: Auto-analysis failed, but you can still use the extension manually.",
          { modal: false }
        );
      }
    })(); // Run immediately
    ...
    context.subscriptions.push(
      vscode.commands.registerCommand("codedoc.testExtension", () => {
        ...
        vscode.window.showInformationMessage(
          "✅ CodeDoc extension is working! Check console for logs."
        );
      })
    );
    ...
    context.subscriptions.push(
      vscode.commands.registerCommand("codedoc.testMarkdown", () => {
    ...
    context.subscriptions.push(
      vscode.commands.registerCommand("codedoc.installBrowser", async () => {
        ...
        try {
          ...
          if (isAvailable) {
            vscode.window
              ...
              .then((choice) => {
                if (choice === "Test Image Export") {
                  vscode.window.showInformationMessage(
                    "Generate a diagram first, then try the 'Open as Image' button to test the feature.",
                    "OK"
                  );
                }
              });
            ...
            const result = await vscode.window.showWarningMessage(
              "❌ Chrome browser still not found. This can happen if:\n" +
    ...
    context.subscriptions.push(
      vscode.commands.registerCommand(
        "codedoc.puppeteerTroubleshoot",
        async () => {
          console.log("codedoc.puppeteerTroubleshoot command executed");

          const result = await vscode.window.showInformationMessage(
            "Puppeteer Chrome Installation Troubleshooting",
            {
              modal: true,
              detail:
                "Chrome may be installed but VS Code can't access it due to user context differences.\n\n" +
          ...
          if (result === "Open Terminal") {
            ...
            vscode.env.openExternal(
              vscode.Uri.parse("https://pptr.dev/troubleshooting")
            );
          }
        }
      )
    ...
    context.subscriptions.push(
      vscode.commands.registerCommand("codedoc.testBrowserError", async () => {
        console.log(
          "codedoc.testBrowserError command executed - simulating browser launch failure"
        );

        // Test the browser launch and error handling
        try {
          const result = await convertMermaidToSvg(
            "classDiagram\nclass TestClass {\n  +testMethod()\n}"
          );
          vscode.window.showInformationMessage(
            "✅ Browser test successful! Image export should work."
          );
        } catch (error) {
          vscode.window.showErrorMessage(
            `❌ Browser test failed: ${
    ...
    context.subscriptions.push(
      vscode.commands.registerCommand("codedoc.testAutoAnalysis", async () => {
        ...
        try {
          ...
          if (workspaceFolders && workspaceFolders.length > 0) {
            ...
            if (javaFiles.length > 0) {
              ...
              await vscode.commands.executeCommand("codedoc.visualizeCode");
              vscode.window.showInformationMessage(
                `✅ Manual auto-analysis completed! Found ${javaFiles.length} Java files.`
              );
            } else {
              vscode.window.showWarningMessage(
                "⚠️ No Java files found in workspace."
              );
            }
          } else {
...
<svg xmlns="http://www.w3.org/2000/svg" width="800" height="600">
  ...
  <text x="400" y="400" font-family="Arial" font-size="12" fill="#888" text-anchor="middle">Error: ${
    ...
    case "Install Chrome":
      ...
      if (installResult === "Open Terminal") {
        ...
        setTimeout(() => {
          vscode.window
            .showInformationMessage(
              "Chrome installation started. After installation completes, you may need to restart VS Code.",
              "Restart VS Code",
              "Check Status",
              "OK"
            )
