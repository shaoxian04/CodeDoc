...
class OpenAIService {
    ...
    createClassDocumentationPrompt(javaClass, relatedClasses) {
        ...
        prompt += `Format the response in clear markdown with appropriate headers and sections.`;
        return prompt;
    }
    createProjectOverviewPrompt(structure) {
        ...
        prompt += `5. Potential areas for improvement or refactoring\n`;
        prompt += `6. Technology stack insights based on the structure\n`;
        return prompt;
    }
    createContextPrompt(structure) {
        ...
        structure.classes.forEach(cls => {
            ...
            if (cls.isController && cls.endpoints.length > 0) {
