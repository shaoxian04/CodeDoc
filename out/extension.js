...
function activate(context) {
    ...
    try {
        ...
        setTimeout(async () => {
            try {
                console.log("🔍 [PUPPETEER-CHECK] Starting Puppeteer availability check...");
                const isPuppeteerAvailable = await checkPuppeteerAvailability();
                if (isPuppeteerAvailable) {
                    console.log("✅ [PUPPETEER-CHECK] Puppeteer is available - 'Open as Image' feature enabled");
                    vscode.window.showInformationMessage("CodeDoc: Chrome browser detected. 'Open as Image' feature is ready!", { modal: false });
                }
                else {
                    console.log("⚠️ [PUPPETEER-CHECK] Puppeteer not available - prompting user");
                    await promptPuppeteerInstallation(context);
                }
            }
            catch (error) {
                console.error("❌ [PUPPETEER-CHECK] Error during Puppeteer check:", error);
            }
        ...
        context.subscriptions.push(vscode.commands.registerCommand("codedoc.syncDocs", async (changedFiles) => {
            ...
            try {
                ...
                if (changedFiles &&
                    ...
                    for (const cf of changedFiles) {
                        try {
                            const absPath = cf.startsWith("/") || cf.indexOf("\\:") >= 0
                                ? cf
                ...
                for (const mdUri of filesToCheck) {
                    try {
                        ...
                        const existingSnippet = existing.length > 3000
        ...
        context.subscriptions.push(vscode.commands.registerCommand("codedoc.installBrowser", async () => {
            ...
            // Show installation progress and offer verification
            const result = await vscode.window.showInformationMessage("Installing Chrome browser for diagram export. This may take a few minutes...", "Check Status After Install", "OK");
            if (result === "Check Status After Install") {
                // Wait a bit then check status
                setTimeout(async () => {
                    await vscode.commands.executeCommand("codedoc.checkPuppeteer");
                }, 2000);
            }
        }));
        // Command to check Puppeteer availability
        context.subscriptions.push(vscode.commands.registerCommand("codedoc.checkPuppeteer", async () => {
            console.log("codedoc.checkPuppeteer command executed");
            try {
                const isAvailable = await checkPuppeteerAvailability();
                if (isAvailable) {
                    vscode.window
                        .showInformationMessage("✅ Chrome browser is available! 'Open as Image' feature is ready to use.", "Test Image Export", "OK")
                        .then((choice) => {
                        if (choice === "Test Image Export") {
                            vscode.window.showInformationMessage("Generate a diagram first, then try the 'Open as Image' button to test the feature.", "OK");
                        }
                    });
                }
                else {
                    const result = await vscode.window.showWarningMessage("❌ Chrome browser still not found. This can happen if:\n" +
                        "• Installation is still in progress\n" +
                        "• VS Code needs to be restarted\n" +
                        "• Installation failed or was cancelled", "Restart VS Code", "Reinstall Chrome", "Troubleshoot", "OK");
                    if (result === "Restart VS Code") {
                        vscode.commands.executeCommand("workbench.action.reloadWindow");
                    }
                    else if (result === "Reinstall Chrome") {
                        await promptPuppeteerInstallation(context);
                    }
                    else if (result === "Troubleshoot") {
                        vscode.env.openExternal(vscode.Uri.parse("https://pptr.dev/troubleshooting#chrome-headless-doesnt-launch-on-windows"));
                    }
                }
            }
            catch (error) {
                vscode.window.showErrorMessage(`Error checking Chrome browser: ${error instanceof Error ? error.message : "Unknown error"}`, "OK");
            }
        }));
        // Command to show alternative Puppeteer installation methods
        context.subscriptions.push(vscode.commands.registerCommand("codedoc.puppeteerTroubleshoot", async () => {
            console.log("codedoc.puppeteerTroubleshoot command executed");
            const result = await vscode.window.showInformationMessage("Puppeteer Chrome Installation Troubleshooting", {
                modal: true,
                detail: "Chrome may be installed but VS Code can't access it due to user context differences.\n\n" +
                    "COMMON ISSUE: Chrome installed for different user\n" +
                    "• Terminal shows: C:\\Users\\user\\.cache\\puppeteer\\chrome\\...\n" +
                    "• VS Code looks in: C:\\Users\\[your-username]\\.cache\\puppeteer\\...\n\n" +
                    "SOLUTIONS:\n" +
                    "1. Restart VS Code completely (most common fix)\n" +
                    "2. Run installation in VS Code's integrated terminal:\n" +
                    "   • Use Terminal → New Terminal in VS Code\n" +
                    "   • Run: npx puppeteer browsers install chrome\n\n" +
                    "3. Check installation location:\n" +
                    "   • Run: npx puppeteer browsers list\n" +
                    "   • Note the path shown\n\n" +
                    "4. Alternative: Use other export options\n" +
                    "   • 'Export as .md' works without Chrome\n" +
                    "   • 'Copy' button also works",
            }, "Open Terminal", "Check Status", "Learn More");
            if (result === "Open Terminal") {
                ...
                vscode.env.openExternal(vscode.Uri.parse("https://pptr.dev/troubleshooting"));
            }
        }));
        ...
        context.subscriptions.push(vscode.commands.registerCommand("codedoc.exportClassDocs", async (content) => {
            ...
            try {
                ...
                vscode.window.showInformationMessage(`Class documentation saved to ${fileUri.fsPath}`);
                // Optionally open the file after saving
                const openFile = await vscode.window.showInformationMessage("Class documentation exported successfully!", "Open File");
        ...
        context.subscriptions.push(vscode.commands.registerCommand("codedoc.generateDiagram", async (params) => {
            console.log("codedoc.generateDiagram command executed", params);
            try {
                const config = vscode.workspace.getConfiguration("codedoc");
                const apiKey = config.get("openaiApiKey");
                if (!apiKey) {
                    vscode.window.showErrorMessage("OpenAI API key not configured. Please configure it in the settings.");
                    return;
                }
                ...
                if (response.success && response.data) {
                    ...
                    vscode.window.showErrorMessage(response.error || "Failed to generate diagram");
                }
            }
            catch (error) {
        ...
        context.subscriptions.push(vscode.commands.registerCommand("codedoc.previewDiagram", async (diagramData) => {
...
async function configureModel() {
    ...
    if (selection) {
        await vscode.workspace
            .getConfiguration("codedoc")
            .update("openaiModel", selection.value, vscode.ConfigurationTarget.Global);
        return true;
    }
    return false;
}
async function configureMaxTokens() {
    ...
    if (maxTokens) {
        await vscode.workspace
            .getConfiguration("codedoc")
            .update("maxTokens", parseInt(maxTokens), vscode.ConfigurationTarget.Global);
        return true;
    }
    return false;
}
async function configureTemperature() {
    ...
    if (temperature) {
        await vscode.workspace
            .getConfiguration("codedoc")
            .update("temperature", parseFloat(temperature), vscode.ConfigurationTarget.Global);
        return true;
    }
    return false;
}
...
async function convertMermaidToSvg(mermaidCode) {
    try {
        ...
        try {
            ...
            console.log("✅ [PUPPETEER-CHECK] Puppeteer library imported for image export");
        }
        catch (error) {
  ...
  <text x="400" y="360" font-family="Arial" font-size="14" fill="white" text-anchor="middle">Please check your Puppeteer installation</text>
    ...
    `;
        ...
        try {
            ...
            vscode.window
                .showErrorMessage("Failed to export diagram as image. Chrome browser not found.", "Install Chrome", "Check Status", "Troubleshoot", "Learn More")
                .then((selection) => {
                if (selection === "Install Chrome") {
                    ...
                    vscode.commands.executeCommand("codedoc.puppeteerTroubleshoot");
                }
...
async function checkPuppeteerAvailability() {
    try {
        console.log("🔍 [PUPPETEER-CHECK] Checking Puppeteer browser availability...");
        ...
        // Check if we can find Chrome in common locations
        const os = require('os');
        const path = require('path');
        const fs = require('fs');
        const possibleCachePaths = [
            path.join(os.homedir(), '.cache', 'puppeteer'),
            path.join(os.homedir(), '.puppeteer'),
            path.join(process.env.APPDATA || os.homedir(), 'puppeteer'),
            'C:\\Users\\user\\.cache\\puppeteer', // From your terminal output
        ];
        console.log("🔍 [PUPPETEER-CHECK] Checking possible Chrome locations:");
        for (const cachePath of possibleCachePaths) {
            ...
            try {
                ...
                console.log(`   ❌ Cannot access ${cachePath}`);
            }
        }
        // Try to launch browser to verify it's actually available
        console.log("🚀 [PUPPETEER-CHECK] Attempting to launch browser...");
        ...
        console.log("❌ [PUPPETEER-CHECK] Puppeteer not available:", error instanceof Error ? error.message : "Unknown error");
        console.log("❌ [PUPPETEER-CHECK] Error details:", error);
        return false;
    }
}
async function promptPuppeteerInstallation(context) {
    // Check if user has already dismissed this prompt
    const dismissed = context.globalState.get("puppeteerPromptDismissed", false);
    if (dismissed) {
        console.log("ℹ️ [PUPPETEER-CHECK] User previously dismissed Puppeteer installation prompt");
        return;
    }
    const result = await vscode.window.showInformationMessage("CodeDoc: Chrome browser is required for 'Open as Image' feature. Would you like to install it?", {
        modal: false,
        detail: "This will download Chrome browser (~100MB) for diagram image export. You can skip this and use other export options.",
    }, "Install Chrome", "Learn More", "Skip");
    switch (result) {
        case "Install Chrome":
            console.log("👤 [PUPPETEER-CHECK] User chose to install Chrome");
            // Show installation instructions
            const installResult = await vscode.window.showInformationMessage("Installing Chrome browser for CodeDoc...", {
                modal: false,
                detail: "A terminal will open with the installation command. Please wait for it to complete.",
            }, "Open Terminal", "Cancel");
            if (installResult === "Open Terminal") {
                const terminal = vscode.window.createTerminal("Install Chrome for CodeDoc");
                terminal.show();
                terminal.sendText("npx puppeteer browsers install chrome");
                // Show follow-up message with restart option
                setTimeout(() => {
                    vscode.window
                        .showInformationMessage("Chrome installation started. After installation completes, you may need to restart VS Code.", "Restart VS Code", "Check Status", "OK")
                        .then((choice) => {
        ...
        case "Skip":
        default:
            console.log("👤 [PUPPETEER-CHECK] User chose to skip Chrome installation");
            // Remember user's choice to not prompt again
            await context.globalState.update("puppeteerPromptDismissed", true);
            vscode.window.showInformationMessage("Chrome installation skipped. You can still export diagrams as markdown or copy to clipboard.", "OK");
            break;
    }
}
