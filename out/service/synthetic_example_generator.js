...
class SyntheticExampleGenerator {
    ...
    generateClassUsageExamples(javaClass, springPatterns) {
        const examples = [];
        if (springPatterns.length > 0) {
    ...
    generateBasicMethodExample(javaClass, method) {
    ...
    generateSpringMethodExample(javaClass, method, springPattern) {
        const instanceName = this.getInstanceName(javaClass.name);
        const parameters = this.generateParameterValues(method.parameters);
        const paramString = parameters.join(', ');
        let codeSnippet;
    ...
    generateErrorHandlingExample(javaClass, method) {
        const instanceName = this.getInstanceName(javaClass.name);
        const parameters = this.generateParameterValues(method.parameters);
        const paramString = parameters.join(', ');
        const codeSnippet = `
try {
    ...
    generateSpringInjectionExample(javaClass, springPattern) {
        const instanceName = this.getInstanceName(javaClass.name);
        const codeSnippet = `
@Autowired
...
public void someMethod() {
    ...
    generateConstructorExample(javaClass) {
        const instanceName = this.getInstanceName(javaClass.name);
        const codeSnippet = `
...
${instanceName}.someMethod();`.trim();
    ...
    getVariableName(type) {
        ...
        if (lowerType.includes('user'))
            return 'user';
        if (lowerType.includes('response'))
            return 'response';
        return 'result';
    }
    getHttpMethod(method) {
        const annotations = method.annotations.join(' ').toLowerCase();
        if (annotations.includes('getmapping') || annotations.includes('get'))
            return 'GET';
        if (annotations.includes('postmapping') || annotations.includes('post'))
            return 'POST';
        if (annotations.includes('putmapping') || annotations.includes('put'))
            return 'PUT';
        if (annotations.includes('deletemapping') || annotations.includes('delete'))
            return 'DELETE';
        return 'GET';
    }
}
exports.SyntheticExampleGenerator = SyntheticExampleGenerator;
//# sourceMappingURL=synthetic_example_generator.js.map